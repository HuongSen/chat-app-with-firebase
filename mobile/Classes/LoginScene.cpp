...
bool LoginScene::init()
{
	...
	auto bg = rootNode->getChildByName("background");
	m_scale = bg->getContentSize().width / visibleSize.width;
	bg->setScale(1/m_scale);

	auto username = rootNode->getChildByName("username");
	username->setScaleX(1/m_scale);
	txt_username = ui::TextField::create("", "", 35);
	txt_username->setAnchorPoint(Vec2(0, 0.5));
	...
	txt_username->setPosition(Vec2(username->getContentSize().width*0.1, username->getContentSize().height*0.5));
	txt_username->setTextColor(Color4B::BLACK);
	username->addChild(txt_username);

	auto password = rootNode->getChildByName("password");
	password->setScaleX(1/m_scale);
	txt_password = ui::TextField::create("", "", 35);
	txt_password->setAnchorPoint(Vec2(0, 0.5));
	...
	txt_password->setPosition(Vec2(password->getContentSize().width*0.1, password->getContentSize().height*0.5));
	...
	btn_getpass->setScale(m_scale);
	btn_login = static_cast<ui::Button*>(rootNode->getChildByName("btn_login"));
	btn_login->setScale(m_scale);
    return true;
}
...
void LoginScene::RegisterAccount()
{
	...
	auto txt_username = ui::TextField::create("", "UTM Cafeta.ttf", 20);
	txt_username->setPlaceHolder("username");
	txt_username->setPlaceHolderColor(Color4B::GRAY);
	txt_username->setContentSize(Size(username->getContentSize().width*0.8, username->getContentSize().height*0.8));
	txt_username->setPosition(Vec2(username->getContentSize().width*0.1, username->getContentSize().height*0.35));
	txt_username->setTextColor(Color4B::BLACK);
	username->addChild(txt_username);

	auto password = nodeRegister->getChildByName("password");
	auto txt_password = ui::TextField::create("", "UTM Cafeta.ttf", 20);
	txt_password->setPlaceHolder("password");
	txt_password->setPasswordEnabled(true);
	txt_password->setPlaceHolderColor(Color4B::GRAY);
	txt_password->setContentSize(Size(password->getContentSize().width*0.8, password->getContentSize().height*0.8));
	txt_password->setPosition(Vec2(password->getContentSize().width*0.1, password->getContentSize().height*0.35));
	txt_password->setTextColor(Color4B::BLACK);
	password->addChild(txt_password);

	auto confirm_password = nodeRegister->getChildByName("confirm_password");
	auto txt_confirm_password = ui::TextField::create("", "UTM Cafeta.ttf.ttf", 20);
	...
	txt_confirm_password->setPosition(Vec2(password->getContentSize().width*0.1, password->getContentSize().height*0.35));
	...
	auto txt_email = ui::TextField::create("", "UTM Cafeta.ttf.ttf", 20);
	...
	txt_email->setPosition(Vec2(password->getContentSize().width*0.1, password->getContentSize().height*0.35));
